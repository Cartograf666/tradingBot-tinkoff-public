...
export type StudyOperation = 'campaign' | 'preflight' | 'smoke' | 'observe' | 'continuous-pilot' | 'freeze' | 'status' | 'report';
...
export function resolveStudyHost(environment: NodeJS.ProcessEnv, operation: StudyOperation): string {
  ...
  if (operation === 'preflight' || operation === 'smoke' || operation === 'observe' || operation === 'continuous-pilot' || operation === 'campaign') {
...
async function main() {
  ...
  if (!['campaign', 'preflight', 'smoke', 'observe', 'continuous-pilot', 'freeze', 'status', 'report'].includes(operation)) throw new StudyHostError('Unknown study operation');
